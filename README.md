...
Lints **and auto-repairs** [LeRobotDataset](https://github.com/huggingface/lerobot) datasets (v2.0–v3.0) against their own declared metadata — 16 checks across structural, semantic, temporal, statistical, and video categories, with terminal / JSON / HTML / SARIF reports that drop straight into CI. v0.2 adds `trajlens fix` (repair engine: timestamp de-drift, stats recompute, episode reindex) and `trajlens web`, a CSP-hardened localhost dashboard. v0.3 adds per-episode trust scoring, `--share` redacted issue reports, three new fixers (task-index repair, video metadata sync, orphan shard quarantine), and `--only`/`--except` fixer selection. `pip install trajlens`. Validated by auditing a random sample of 100 public Hugging Face Hub datasets: two known upstream `lerobot` bugs were found in **~19%** and **~3%** of successfully linted datasets.
