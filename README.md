...
> A few research repos below are private until their papers are submitted or published. Links go live with the papers.
...
Lints and auto-repairs [LeRobotDataset](https://github.com/huggingface/lerobot) datasets (v2.0 to v3.0) against their own declared metadata. 16 checks across structural, semantic, temporal, statistical, and video categories with terminal / JSON / HTML / SARIF reports for CI. v0.2 adds `trajlens fix` (timestamp de-drift, stats recompute, episode reindex) and `trajlens web`, a CSP-hardened localhost dashboard. v0.3 adds per-episode trust scoring, `--share` redacted issue reports, three new fixers (task-index repair, video metadata sync, orphan shard quarantine), and `--only`/`--except` fixer selection. `pip install trajlens`. Auditing 100 public Hugging Face Hub datasets surfaced two upstream lerobot bugs present in roughly 19% and 3% of linted datasets.
...
Visual studio for testing and comparing LLM prompts across five providers (Ollama, OpenAI, Anthropic, Groq, Hugging Face). Runs entirely on your machine; keys never leave localhost. Four metrics: exact, fuzzy, local-embedding cosine, and LLM-judge. Clone to scored run in under five minutes.
...
| Work | Area | Status |
|---|---|---|
| [MRI Reconstruction](https://github.com/Kunal-Somani/MRI_Reconstruction): dual-branch physics-guided deep learning; learned gating routes k-space adaptively, no anatomy labels at inference. +1.78% SSIM, 112 ms/slice | Medical imaging | Paper accepted at IC3 (IEEE/ACM) · Dr. Anurag Tiwari, TIET |
| [ThermoBridge](https://github.com/Kunal-Somani/thermobridge) 🔒: bidirectional 3D MRI to CT synthesis via I2SB Schrodinger bridge diffusion, N-way sparse anatomy routing gate, 3D Perona-Malik transformer mixer. 164.9 HU MAE, 34% per-anatomy error reduction | Medical imaging | Paper in preparation, targeting ICASSP 2027 · Dr. Anurag Tiwari, TIET |
| Multi-Modal Sensor Fusion for SLAM in Visually Degraded SAR Environments: pipeline-oriented survey of 55 systems (2011 to 2025) | SAR robotics | Survey submitted · Dr. Ankit Soni, TIET |
| SAR rover 🔒: simulated multi-modal (audio-visual-thermal) search-and-rescue platform | SAR robotics | Journal paper in progress · Dr. Ankit Soni, TIET |
| [Parkinson's early detection](https://github.com/eshaansingla/ParkinsonsEarlyPrediction): CNN on voice + DNN on MPU9250 tremor, late fusion 88% to 91.8%, ESP32 sensor-to-inference pipeline | Multimodal ML | ELC summer research, TIET |
...
PPO-trained locomotion (200K timesteps, PyBullet) to ROS 2 sensor stack (IMU / RPLiDAR / 4x BLDC encoders) to full Isaac Sim 5.1 simulation with live sensor feeds. Currently in sim-to-real transfer; physical rover roughly half built. Team of 5.
...
ROS 2 Humble skid-steer robot that navigates without SLAM: FSM + PD wall-following on 360 degree LiDAR, HSV + depth target tracking, tested in the AWS Small Warehouse Gazebo world.
...
| Project | Description |
|---|---|
| [Archon](https://github.com/Kunal-Somani/archon) | Natural-language brief to live deployed site: hybrid RAG (Cohere dense + BM25, RRF fusion) feeds grammar-constrained code generation<br>Celery async workers, Redis Pub/Sub live WebSocket logs, GitHub App deployer with short-lived tokens, full Prometheus/Grafana/OTel observability |
